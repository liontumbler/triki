...
class Triki {
    ...
    static jugadorActual: string;
    static solo = false;
    constructor(idElement: string, solo = false) {
        Triki.solo = solo;
    ...
    static manejarCellClick(event: any) {
    ...
    static botMove() {
        const emptyCells = Triki.board.reduce((acc: number[], cell, index) => {
            if (cell === '') {
                acc.push(index);
            }
            return acc;
